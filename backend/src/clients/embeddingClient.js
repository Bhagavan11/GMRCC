...
let embedderPipeline = null; // Cache for the model pipeline

/**
 * Loads the embedding model pipeline if not already loaded.
 * Uses 'Xenova/all-MiniLM-L12-v2' for high accuracy and self-hosted cost-free embeddings.
 * @returns {Promise<any>} The loaded pipeline instance.
 */
export async function loadEmbeddingModelPipeline() {
  if (!embedderPipeline) {
 ...
 * Generates an embedding vector for a given text.
 * @param {string} text - The text to embed.
 * @returns {Promise<number[]>} An array of floats representing the embedding.
 */
export async function generateEmbedding(text) {
  if (!text || text.trim().length === 0) {
    throw new Error("⚠️ Cannot generate embedding: input text is empty.");
  }
...
console.log("embedded text")
  return Array.from(output.data); // Convert Float32Array -> plain JS array
}
