...
import { scrapeAllResearchInfo } from './scraping/researchInfoScraper.js';
console.log("--- scraper.js: researchInfoScraper imported. ---");

import { scrapeAllRankingInfo } from './scraping/rankingInfoScraper.js';
console.log("--- scraper.js: rankingInfoScraper imported. ---");    


console.log("--- scraper.js: All initial imports successfully completed. ---");

import { scrapeAllPlacementInfo } from './scraping/placementScraper.js';
console.log("--- scraper.js: placementScraper imported. ---");

import { scrapeAllOtherInfo } from './scraping/otherInfoScraper.js';
console.log("--- scraper.js: otherInfoScraper imported. ---");

import { scrapeAllAdditionalInfo } from './scraping/additionalInfoScraper.js';
console.log("--- scraper.js: additionalInfoScraper imported. ---");

import { scrapeAllDepartments } from './scraping/departmentScraper.js';
console.log("--- scraper.js: departmentScraper imported. ---");
import{scrapeStudentBodiesInfo} from './scraping/StudentClubsBodiles.js';
...
export async function startScraping() {
    ...
    console.log(`\n--- Scraping student-related info completed. ---`);


        // Student Clubs and Bodies
    console.log(`--- startScraping: Scraping student clubs and bodies info. ---`);
    const studentClubsDocs = await scrapeStudentBodiesInfo();
    studentClubsDocs.forEach(doc => {
        if (doc && doc.metadata) {
            doc.metadata.docId = uuidv4();
        }
    });
    scrapedDocuments.push(...studentClubsDocs);

    // Research Information
    console.log(`--- startScraping: Scraping research information. ---`);
    const researchDocs = await scrapeAllResearchInfo();
    researchDocs.forEach(doc => {
        if (doc && doc.metadata) {
            doc.metadata.docId = uuidv4();
        }
    });
    scrapedDocuments.push(...researchDocs);

    // Ranking Information
    console.log(`--- startScraping: Scraping ranking information. ---`);
    const rankingDocs = await scrapeAllRankingInfo();
    rankingDocs.forEach(doc => {
        if (doc && doc.metadata) {
            doc.metadata.docId = uuidv4();
        }
    });
    scrapedDocuments.push(...rankingDocs);

    // Placement Information
    console.log(`--- startScraping: Scraping placement information. ---`);
    const placementDocs = await scrapeAllPlacementInfo();
    placementDocs.forEach(doc => {
        if (doc && doc.metadata) {
            doc.metadata.docId = uuidv4();
        }
    });
    scrapedDocuments.push(...placementDocs);

    // Other Information
    console.log(`--- startScraping: Scraping other information. ---`);
    const otherInfoDocs = await scrapeAllOtherInfo();
    otherInfoDocs.forEach(doc => {
        if (doc && doc.metadata) {
            doc.metadata.docId = uuidv4();
        }
    });
    scrapedDocuments.push(...otherInfoDocs);

    // Additional Information
    console.log(`--- startScraping: Scraping additional information. ---`);
    const additionalInfoDocs = await scrapeAllAdditionalInfo();
    additionalInfoDocs.forEach(doc => {
        if (doc && doc.metadata) {
            doc.metadata.docId = uuidv4();
        }
    });
    scrapedDocuments.push(...additionalInfoDocs);

    // Department Information
    console.log(`--- startScraping: Scraping department information. ---`);
    const departmentDocs = await scrapeAllDepartments();
    departmentDocs.forEach(doc => {
        if (doc && doc.metadata) {
            doc.metadata.docId = uuidv4();
        }
    });
    scrapedDocuments.push(...departmentDocs);
